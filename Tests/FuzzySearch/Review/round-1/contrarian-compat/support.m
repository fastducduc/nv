...
#include "persistence-models.inc"
...
@end
@interface StateField : NSObject { @public StateEditor *editor; NSString *value; NSMutableArray *followedLinks; NSString *snapbackString, *snapbackSearchMode; StateController *controller; }
- (id)currentEditor;
...
- (void)selectText:(id)sender {}
- (void)dealloc { [NSObject cancelPreviousPerformRequestsWithTarget:self]; [followedLinks release]; [snapbackString release]; [snapbackSearchMode release]; [value release]; [super dealloc]; }
@end
...
@public NVBrowserSession *notationController;
    StateField *field; StateEditor *textView; id noteTitleField, noteTagsField; NSArray *savedSelectedNotes; StateWindow *window; StateTable *notesTableView;
...
- (NSInteger)numberOfRows { return [[owner->notationController notesListDataSource] count]; }
- (id)dataSource { return [owner->notationController notesListDataSource]; }
...
- (id)init {
    if ((self = [super init])) {
        field = [StateField new]; field->controller = self; field->followedLinks = [NSMutableArray new]; field->editor = [[StateEditor new] autorelease];
...
- (void)revealToolbar {}
#define NSUserDefaults ReviewDefaults
...
@implementation StateField (ProductionLinks)
#include "dual-field.inc"
@end
#undef NVControllerForView
