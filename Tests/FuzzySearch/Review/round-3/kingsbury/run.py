...
p.add_argument('--expect-fixed', action='store_true', help='Compatibility option; all assertions require correct behavior.')
a = p.parse_args()
out = ROOT / 'build/FuzzySearchReview/round-3/kingsbury' / ('sanitize' if a.sanitize else 'native')
...
(out / 'state-probe.m').write_text(prefix + helpers + validation + (HERE / 'support.m').read_text() + (HERE / 'cases.m').read_text())
...
objects = []
production_inputs = sorted(set([*groups, *sources[:-1],
    'Sources/Browser/AppController.h', 'Sources/Browser/NVBrowserSession.h',
    'Sources/Search/NVSearchService.h', 'Sources/Search/NVSearchQuery.h', 'Sources/Search/NVSearchCorpus.h']))
before_hashes = {path: hashlib.sha256((ROOT / path).read_bytes()).hexdigest() for path in production_inputs}
baseline = '11f571f87270cfebcb1b4f9537c88b52f87f77aa'
baseline_matches = all(hashlib.sha256(subprocess.check_output(['git', 'show', baseline + ':' + path], cwd=ROOT)).hexdigest() == digest
...
inputs = [*groups, 'Tests/FuzzySearch/Browser/browser-tests.m', str(HERE.relative_to(ROOT) / 'support.m'), str(HERE.relative_to(ROOT) / 'cases.m'), str(Path(__file__).relative_to(ROOT)), *sources[:-1]]
record = {'command': ['python3', str(Path(__file__).relative_to(ROOT)), *(['--sanitize'] if a.sanitize else []), *(['--expect-fixed'] if a.expect_fixed else [])],
          ...
          'production_hashes_before_compile': before_hashes,
          'production_baseline': baseline, 'production_inputs_match_baseline': baseline_matches,
          'production_inputs_stable': all(hashlib.sha256((ROOT / path).read_bytes()).hexdigest() == digest for path, digest in before_hashes.items()),
