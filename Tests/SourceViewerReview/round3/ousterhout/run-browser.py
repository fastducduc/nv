#!/usr/bin/env python3
"""Check the browser consumer of joined Find state in a copied app with disposable notes."""
import os
import sys
...
with tempfile.TemporaryDirectory(prefix='nvalt-window-tests-') as root:
    ...
    harness.write_text((repo / 'Tests/Regression/source-workflow/support.h').read_text() + prefix + Path(os.environ.get('NV_REVIEW_BODY_PATH', str(Path(__file__).with_name('browser-probe.m')))).read_text())
